...
const TemplateCard = ({
  ...
  return (
    <figure
      className={cn(
        "relative w-[400px] cursor-pointer overflow-hidden rounded-xl p-4"
      )}
    >
      <motion.div whileHover={{ scale: 1.06 }} className="relative shadow-xl">
        <div className="h-[30vh] w-full  rounded-xl">
          <Image
            src={src}
            alt="bg image"
            layout="fill"
        ...
        <div className="w-full flex items-center justify-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <p
            className={` text-3xl font-custom font-semibold text-${textColor} `}
          >
            {text}
          </p>
        </div>
